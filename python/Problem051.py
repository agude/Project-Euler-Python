...
NOTE: That "part of an eight prime value family" means the prime must be one of
the numbers generated by the substitution.
"""
...
def problem_051():
    ...
    while True:
        ...
        for base_prime in primes:
            # We know the largest prime of the set with 7 primes is 56773, so
            # start there as any with 8 would have been found already if they
            # were smaller.
            ...
            for number_to_replace in range(1, len(base_tuple)):
                for digits_to_replace in combinations(digits, number_to_replace):
                    ...
                    # If we have 8 or more primes, we are done
                    if len(new_primes) >= 8:
                        end_time = time() - start_time
                        # We note a special stipulation: the answer must be one
                        # of the numbers in the set generated by the
                        # substitution.
                        #
                        # For example, 120383, when masked as *2*3*3, yields:
                        #
                        # 121313
                        # 222323
                        # 323333
                        ...
                        # 929393
                        #
                        # However, 120383 is not in the generated set, so even
                        # though it is prime and smaller than any number in the
